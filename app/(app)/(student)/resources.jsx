import { View, Text, useColorScheme, FlatList, TouchableOpacity, Linking } from 'react-native'
          ...
          <View className='px-5'>
            <Text style={{fontFamily: (fontsLoaded ? 'Poppins_600SemiBold' : undefined), fontSize: hp(2), color: theme.header}} className='self-center mt-3'>{item.category}</Text>
            {
              item.contents.map((content, index) => (
                <TouchableOpacity onPress={() => handleSelect(content.link, content.title)} key={index} style={{backgroundColor: theme.cardBackground}} className='rounded-3xl my-1 p-3'>
